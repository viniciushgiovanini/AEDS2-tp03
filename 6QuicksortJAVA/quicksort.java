...
//RESULTADO DO PROGRAMA: O programa deu 60%, não sei porque está dando erro, ele roda perfeitamente com poucos dados, porém quando se insere muitos dados o quicksort obtem pequenos erros
//ele faz 3 quicks, 1 quick para o canada, 1 quick para o EUA, 1 quick para o reino unido.
...
public class quicksort {
  ...
  public static void ordenarPAIS(String[] todasasentradas, String[] paisDeorigem, String[] nomedaSerie, int esq,
      int dir) {// Explicação
    // dessa funcao
    // no
    // cabeçalho.
    ...
    int i = esq;
    int j = dir;
    String pivo = paisDeorigem[(esq + dir) / 2];
    // int pivoNUM = (esq + dir) / 2;
    int primeiroWHILE = 100;
    int segundoWHILE = 100;

    while (i <= j) {
      primeiroWHILE = 0;
      while (primeiroWHILE == 0) {

        primeiroWHILE = palavrasCRESCENTES(paisDeorigem[i], pivo);
        if (primeiroWHILE == 0) {
      ...
      while (segundoWHILE == 0) {
        segundoWHILE = palavrasCRESCENTES(pivo, paisDeorigem[j]);
        if (segundoWHILE == 0) {
          j--;
        }
      ...
      if (i <= j) {
        ...
        swap(paisDeorigem, i, j);
        swap(todasasentradas, i, j);
        swap(nomedaSerie, i, j);
        i++;
        j--;
      }
    ...
    if (esq < j) {
      ...
      ordenarPAIS(todasasentradas, paisDeorigem, nomedaSerie, esq, j);

    }

    if (i < dir) {

      ordenarPAIS(todasasentradas, paisDeorigem, nomedaSerie, i, dir);

    }

  }

  public static void ordenarNome(String[] todasasentradas, String[] paisDeorigem, String[] nomedaSerie, int esq,
      int dir) {

    int i = esq;
    int j = dir;
    String pivo = nomedaSerie[(esq + dir) / 2];
    // int pivoNUM = (esq + dir) / 2;
    int primeiroWHILE = 100;
    int segundoWHILE = 100;
    ...
    while (i <= j) {
      ...
      while (primeiroWHILE == 0) {
        ...
        primeiroWHILE = palavrasCRESCENTES(nomedaSerie[i], pivo);
        if (primeiroWHILE == 0) {
      ...
      segundoWHILE = 0;
      while (segundoWHILE == 0) {
        segundoWHILE = palavrasCRESCENTES(pivo, nomedaSerie[j]);
        if (segundoWHILE == 0) {
          j--;
        }
    ...
    if (esq < j) {
      ...
      ordenarNome(todasasentradas, paisDeorigem, nomedaSerie, esq, j);

    }

    if (i < dir) {

      ordenarNome(todasasentradas, paisDeorigem, nomedaSerie, i, dir);

    }
  }

  public static int contarQuantidadeCanada(String[] paisDeorigem) {

    int qtdELEMENTOS = quantidadeElementos(paisDeorigem);
    int contador = 0;
    for (int i = 0; i < qtdELEMENTOS; i++) {

      if (paisDeorigem[i].contains("Canadá")) {
        contador++;
      }
  ...
  public static int contarQuantidadeEUA(String[] paisDeorigem) {
    ...
    for (int i = 0; i < qtdELEMENTOS; i++) {
      ...
      if (paisDeorigem[i].contains("Estados Unidos")) {
  ...
  public static int contarQuantidadeReinoUnido(String[] paisDeorigem) {
    ...
    for (int i = 0; i < qtdELEMENTOS; i++) {
      ...
      if (paisDeorigem[i].contains("Reino Unido")) {
  ...
  public static void main(String[] args) {
    ...
    SerieQUICK nt = new SerieQUICK();// Faz a ligacao com a classe
    ...
    int dir = quantidadeElementos(paisDeorigem) - 1;

    ordenarPAIS(todasasentradas, paisDeorigem, nomedaSerie, esq, dir);
    esq = 0;
    dir = contarQuantidadeCanada(paisDeorigem) - 1;
    ordenarNome(todasasentradas, paisDeorigem, nomedaSerie, esq, dir);
    esq = dir + 1;
    dir = contarQuantidadeEUA(paisDeorigem) - 1;
    ordenarNome(todasasentradas, paisDeorigem, nomedaSerie, esq, dir);
    esq = dir + 1;
    dir = contarQuantidadeReinoUnido(paisDeorigem) - 1;
    ordenarNome(todasasentradas, paisDeorigem, nomedaSerie, esq, dir);
