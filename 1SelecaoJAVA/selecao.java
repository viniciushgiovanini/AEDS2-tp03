...
//RESULTADO DO PROGRAMA: O programa deu 100%, fazendo todas as operações de ordenação por seleção
//Esse programa tem que ordenar pelo pais, e os paises que forem repetidos devem ser ordenados pelo nome, então eu pego o pais ordeno com a funcao ordenarPais, com essa função ele ordena
//somente com a primeira letra da palavra, da certo pq está presente somente 3 pais, essa função vai ordenando o vetor pais e analisando o vetor pais, porém sempre que ele faz um swap,
//em determinada posicao ele faz no vetor com todas os elementos da classe o mesmo swap e no vetor nome também.
...
public class selecao {
  ...
  public static void ordenarPAIS(String[] todasasentradas, String[] paisDeorigem, String[] nomedaSerie) {// Explicação
                                                                                                         // dessa funcao
                                                                                                         // no
                                                                                                         // cabeçalho.

    char[] pais1 = new char[100];
    int valorpais1;
    char[] pais2 = new char[100];
    int valorpais2;
    int contadorletra = 0;
    int valormenor = 0;
    int posimenor = 0;
    boolean existevalormenor = false;
    int j = 0;
    int qtdELEMENTOS = quantidadeElementos(todasasentradas);
    int primeirofor = qtdELEMENTOS - 1;

    for (int i = 0; i < primeirofor; i++) {

      pais1 = paisDeorigem[i].toCharArray();

      valorpais1 = (int) pais1[contadorletra];
      valormenor = valorpais1;

      for (j = i + 1; j < qtdELEMENTOS; j++) {

        pais2 = paisDeorigem[j].toCharArray();
        valorpais2 = (int) pais2[contadorletra];

        if (valormenor > valorpais2) {

          valormenor = valorpais2;
          posimenor = j;
          existevalormenor = true;

        }

      }

      if (existevalormenor == true) {

        swap(todasasentradas, i, posimenor);
        swap(paisDeorigem, i, posimenor);
        swap(nomedaSerie, i, posimenor);
        existevalormenor = false;
  ...
  public static void ordenarNOME(String[] todasasentradas, String[] paisdeOrigem, String[] nomedaSerie) {// Essa função
                                                                                                         // ordenao nome

    // Essa função pega o nome na posicao 0 bota em uma string pega na posicao 0+1 e
    // bota na segunda string, e manda para a funcao palavras crescentes

    boolean teste = false;

    int qtdELEMENTOS = quantidadeElementos(nomedaSerie);

    String receberPRIMEIRO = " ";
    String receberSEGUNDO = " ";

    int posimenor = 0;
    boolean marcSWAP = false;
    boolean marcsSWAP2 = false;

    for (int i = 0; i < qtdELEMENTOS - 1; i++) {

      if (marcsSWAP2 == false) {// essa receberPrimeiro so vai receber o nome da serie na posicao i quando não
                                // tiver achado um elemento menor que ele, se achar ele vai testar a partir dele
      ...
      for (int j = i + 1; j < qtdELEMENTOS; j++) {
        ...
        if (paisdeOrigem[i].contains(paisdeOrigem[j])) {
          ...
          receberSEGUNDO = nomedaSerie[j];
          teste = palavrasCRESCENTES(receberPRIMEIRO, receberSEGUNDO);// Se a string da esquerda for habilitada pra ser
                                                                      // trocada com a primeira retorna TRUE

          if (teste == true) {// se for true, ele salva a posicao que essa string 2 está no vetor, habilita a
                              // troca (swap), habilita (desabilita) a leitura de uma nova string PRIMEIRO
                              // (que é a string menor), e salva essa string como a menor string encontrada no
                              // momento, da ordem alfabetica, dessa forma, essa string agora em diante vai
                              // ser comparada com as demais posicoes.

            posimenor = j;
            marcSWAP = true;
            marcsSWAP2 = true;
            receberPRIMEIRO = receberSEGUNDO;
  ...
  public static void main(String[] args) {
    ...
    String[] paisDeorigem = new String[100];// vetor com o elementos pais
    String[] nomedaSerie = new String[100];// vetor com elemento nome
    int contadorentrada = 0;
    // entradapasta.append(entrada);
    // System.out.println(entradapasta);

    SerieSELECAO nt = new SerieSELECAO();// Faz a ligacao com a classe
    while (!(entrada.equals("FIM"))) {// roda até entrada ser digitado FIM
      ...
      paisDeorigem[contadorentrada] = nt.getPais();
    ...
    ordenarPAIS(todasasentradas, paisDeorigem, nomedaSerie);
    ordenarNOME(todasasentradas, paisDeorigem, nomedaSerie);
