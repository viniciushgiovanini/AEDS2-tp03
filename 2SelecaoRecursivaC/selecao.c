...
//Programa deu 94% de acerto, não deu 100% pois algumas string estão com alguns espaços, ela foi feita de forma recursiva na ordenacao do PAIS, e tive que fazer trim para retirar os espaco
//presente na serie Grimm no pais tinha um espaco na frente dos EUA que fazia ela ir para a primeira posicao da ordenacao
...
void concatenarPais(char *pais, char receberpais[100][300])
{

  int qtdELEMENTOS = quantidadeElementos(receberpais);

  strcpy(receberpais[qtdELEMENTOS], pais);
}
...
bool palavraCrescente(char *um, char *dois)
{
...
int ordenarPais(char receberPais[100][300], char receberTUDO[100][300], char receberNome[100][300], int i, char *salvar, char *salvar2, char *salvar3)
{
  //Essa funcao esta recursiva, ela é recusiva no loop de selecionar a posicao que está cada string o I, quando acaba a quantidade de elemetos o loop acaba
  int qtdELEMENTOS = quantidadeElementos(receberPais);
  bool marcsSWAP = false;
  bool palavra = false;

  if (i >= (qtdELEMENTOS - 1)) //FLAG RECURSIVO
  {

    return 0;
  }

  salvar = receberPais[i];
  salvar2 = receberTUDO[i];
  salvar3 = receberNome[i];

  for (int j = i; j < qtdELEMENTOS - 1; j++)
  {

    palavra = palavraCrescente(salvar, receberPais[j + 1]);

    if (palavra == true)
    {

      copiarSTR(salvar, receberPais[j + 1]);
      copiarSTR(salvar2, receberTUDO[j + 1]);
      copiarSTR(salvar3, receberNome[j + 1]);
      marcsSWAP = true;
    }
  }
  ...
  if (marcsSWAP == true)
  {
    swap(salvar, receberPais[i]);
    swap(salvar2, receberTUDO[i]);
    swap(salvar3, receberNome[i]);
  }

  return ordenarPais(receberPais, receberTUDO, receberNome, i = i + 1, salvar, salvar2, salvar3);
}

void ordenarNome(char receberPais[100][300], char receberTUDO[100][300], char receberNome[100][300])
{
  ...
  char receberPrimeiro[300];
  char receberSegundo[300];

  int posimenor = 0;
  bool marcSWAP = false;
  bool marcSWAP2 = false;
  ...
  for (int i = 0; i < qtdELEMENTOS - 1; i++)
  {
    if (marcSWAP2 == false)
    {
      callocLinhaString(receberPrimeiro);
      strcpy(receberPrimeiro, receberNome[i]);
    }

    for (int j = i + 1; j < qtdELEMENTOS; j++)
    {
      contains = strstr(receberPais[i], receberPais[j]);

      if (contains)
      {
        callocLinhaString(receberSegundo);
        strcpy(receberSegundo, receberNome[j]);

        teste = palavraCrescente(receberPrimeiro, receberSegundo);
        ...
        if (teste == true)
        {
          ...
          marcSWAP = true;
          marcSWAP2 = true;
          copiarSTR(receberPrimeiro, receberSegundo);
        }
      }
    }
    ...
    if (marcSWAP == true)
    {
      swapMATRIZ(receberTUDO, i, posimenor);
      swapMATRIZ(receberNome, i, posimenor);
      marcSWAP2 = false;
      marcSWAP = false;
    }
  }
...
int main() ////////////////////////////CRIAR FUNCAO PARA PESQUISAR NOME, FORMATO ETC
{
  ...
  char linha[2000];
  char receberpais[100][300];
  char receberTUDO[100][300];
  char receberNOME[100][300];
  callocMatriz(receberpais);
  ...
  while (!(entrada[0] == 'F' && entrada[1] == 'I' && entrada[2] == 'M')) //ESSE WHILE SO PARA QUANDO A STRING DA ENTRADA QUE É O ARQUIVO.HTML FOR ESCRITO FIM
  {
    ...
    trim(Sserie.vetorFINALPAIS); //trim para corrigir a serie Grimm
    concatenarPais(Sserie.vetorFINALPAIS, receberpais);
  ...
  char salvarstr2[300];
  char salvarstr3[300];
  ordenarPais(receberpais, receberTUDO, receberNOME, contadorOrdenarpais, salvarstr, salvarstr2, salvarstr3);
  ordenarNome(receberpais, receberTUDO, receberNOME);
