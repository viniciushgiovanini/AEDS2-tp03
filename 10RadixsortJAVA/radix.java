...
//RESULTADO DO PROGRAMA: Deu erro no tamanho do vetor COUNT E ORDENADO.
...
public class radix {
  ...
  public static void ordenarEP(String[] todasasentradas, int[] numeroEP, String[] nomedaSerie) {// Explicação
    // dessa funcao
    // no
    // cabeçalho.

    int[] count = new int[getMaior(numeroEP)];
    int[] ordenado = new int[quantidadeElementosINT(numeroEP) - 1];

    for (int i = 0; i < count.length; count[i] = 0, i++)
      ;
    for (int i = 0; i < quantidadeElementosINT(numeroEP) - 1; count[i] = 0, i++)
      ;
    for (int i = 1; i < count.length; count[i] += count[i - 1], i++)
      ;
    for (int i = quantidadeElementosINT(numeroEP) - 1; i >= 0; ordenado[count[numeroEP[i]]
        - 1] = numeroEP[i], count[numeroEP[i]]--, i--)
      ;

  }

  public static int getMaior(int[] numeroEP) {

    int qtdELEMENTOS = quantidadeElementosINT(numeroEP);
    int contador = 0;

    for (int i = 0; i < qtdELEMENTOS; i++) {

      if (numeroEP[i] > contador) {
  ...
  public static void main(String[] args) {
    ...
    SerieRADIX nt = new SerieRADIX();// Faz a ligacao com a classe
    ...
    ordenarEP(todasasentradas, numeroEP, nomedaSerie);
