...
//RESULTADO DO PROGRAMA: O programa deu 100%, fazendo todas as operações de ordenação por bolha
...
public class bolha {
  ...
  public static void swapINT(int[] vetor, int posi1, int posi2) {// função para trocar elementos do vetor;.
  ...
  public static void ordenarTEMP(String[] todasasentradas, int[] ntemporadas, String[] nomedaSerie) {// Explicação
                                                                                                     // dessa funcao
                                                                                                     // no
                                                                                                     // cabeçalho.
    int qtdELEMENTOS = quantidadeElementosINT(ntemporadas);
    boolean houvetroca = true;
    for (int rep = 0; rep < qtdELEMENTOS - 1 && houvetroca; rep++) {
      houvetroca = false;
      for (int b = 0; b < qtdELEMENTOS - (rep + 1); b++) {
        if (ntemporadas[b] > ntemporadas[b + 1]) {
          swapINT(ntemporadas, b, b + 1);
          swap(todasasentradas, b, b + 1);
          swap(nomedaSerie, b, b + 1);
          houvetroca = true;
        }
      }
    }
  ...
  public static void ordenarNOME(String[] todasasentradas, int[] ntemporadas, String[] nomedaSerie) {// Essa função
                                                                                                     // ordenao nome

    int qtdELEMENTOS = quantidadeElementos(todasasentradas);

    int avalTROCA = 100;
    for (int rep = 0; rep < qtdELEMENTOS - 1; rep++) {
      boolean houveTroca = false;
      for (int b = 0; b < qtdELEMENTOS - (rep + 1); b++) {
        if (ntemporadas[b] == ntemporadas[b + 1]) {
          avalTROCA = palavrasCRESCENTES(nomedaSerie[b], nomedaSerie[b + 1]);
          if (avalTROCA == 1) {
            swapINT(ntemporadas, b, b + 1);
            swap(todasasentradas, b, b + 1);
            swap(nomedaSerie, b, b + 1);
  ...
  public static void main(String[] args) {
    ...
    while (!(entrada.equals("FIM"))) {// roda até entrada ser digitado FIM
      ...
      ntemporadas[contadorentrada] = nt.getNtemporadasINTEIRO();
    ...
    ordenarTEMP(todasasentradas, ntemporadas, nomedaSerie);
    ordenarNOME(todasasentradas, ntemporadas, nomedaSerie);
